...
import static Hilos.Bus3.posX;
import static Hilos.Bus3.posY;
import static Hilos.Bus3.sending;
import java.io.File;
...
public class Bus5 extends Thread{
    ...
    public void run(){
        ...
        while (counter<3){
            try {
            File file = new File("Buses/Coordenadas/Bus5Fluid.txt");
            ...
            while (entry.hasNext()){
                ...
                if (skip==true){
                    ...
                    if (Bus4.array[0].equals(stop)){
                        System.out.println("Concurrencia resuelta -- Bus 5 retornado a su lugar.");
                        skip = false;
                        sendDet = false;
                        GUI.bus5.setLocation(posX-15, posY-15);
                    }
    ...
    public void sendDetenido() throws IOException{
        GUI.escrituraDatos.writeUTF("Bus 5 - Detenido.");
        GUI.escrituraDatos.flush();
    }
    
    public void enviarCoordenadasBus5() throws IOException{
        
        String parada = "Próxima Parada - Bus 5: ";
