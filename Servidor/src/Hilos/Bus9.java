...
import static Hilos.Bus7.posX;
import static Hilos.Bus7.posY;
import static Hilos.Bus7.sending;
import java.io.File;
...
public class Bus9 extends Thread{
    ...
    public void run(){
        ...
        while (counter<3){
            try {
            File file = new File("Buses/Coordenadas/Bus9Fluid.txt");
            ...
            while (entry.hasNext()){
                ...
                if (skip==true){
                    ...
                    if (Bus8.array[0].equals(stop)){
                        System.out.println("Concurrencia resuelta -- Bus 9 retornado a su lugar.");
                        skip = false;
                        sendDet = false;
                        GUI.bus9.setLocation(posX-15, posY-15);
                    }
    ...
    public void sendDetenido() throws IOException{
        GUI.escrituraDatos.writeUTF("Bus 9 - Detenido.");
        GUI.escrituraDatos.flush();
    }
    
    public void enviarCoordenadasBus9() throws IOException{
        
        String parada = "Próxima Parada - Bus 9: ";
